...
public class ViewToImageUtil {
     ...
     * 存入sd卡
     */
    public static void saveDisk(Context context, Bitmap bitmap, final OnImageSavedCallback mOnImageSavedCallback){
        final String destPath = FileUtil.createImageFile(context).getAbsolutePath();
        MHLogUtil.d("destPath====" + destPath);
        FileUtil.compressBitmapToFile(bitmap, destPath);
        new Handler(Looper.getMainLooper()).post(new Runnable() {
            @Override
...
//                }).launch();
    ...
    public static Bitmap generateBigBitmap(final ViewGroup viewGroup, int width) {
        ...
        MHLogUtil.d(width + "");
        MHLogUtil.d(String.format("width=%s, measuredWidth=%s, height=%s, measuredHeight=%s",
