...
class PushBtnTouchListener implements View.OnTouchListener {
    ...
    private Point minRect;//最小的区域
    private Point maxRect;//最大的区域
    private Point mViewCenter;
    private static final double PI = 3.14159265359;

    public PushBtnTouchListener(View mView, View mDeleteView, Point minRect, Point maxRect) {
        this.mView = mView;
    ...
    public boolean onTouch(View pushView, MotionEvent event) {
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (newHeight > maxRect.y) {
                    newHeight = (int) maxRect.y;
                }
                if (newWidth > maxRect.x) {
                    newWidth = (int) maxRect.x;
                }

                if (newHeight < minRect.y) {
                    newHeight = (int) minRect.y;
                }
                if (newWidth < minRect.x) {
                    newWidth = (int) minRect.x;
                }
