...
public abstract class FlowLayout extends ViewGroup {
    ...
    private boolean isArriveAppointLine = true;
        ...
        for (int i = 0; i < count; i++) {
            ...
            child.measure(
                    getChildMeasureSpec(widthMeasureSpec, this.getPaddingLeft()
                            + this.getPaddingRight(), layoutParams.width),
                    getChildMeasureSpec(heightMeasureSpec, this.getPaddingTop()
                            + this.getPaddingBottom(), layoutParams.height));

            int hSpacing = this.getHorizontalSpacing(layoutParams);
            int vSpacing = this.getVerticalSpacing(layoutParams);
            ...
            boolean newLine = layoutParams.newLine || (mode != MeasureSpec.UNSPECIFIED && lineLength > size);
            if (newLine) {
                lines++;
                if (lines == 4 && isArriveAppointLine) {
                    MHLogUtil.e("FlowLayout","--"+i);
                    isArriveAppointLine = false;
                    getShowedNumber(i);
                }
    ...
    private int getHorizontalSpacing(LayoutParams layoutParams) {
        ...
        if (layoutParams.horizontalSpacingSpecified()) {
            hSpacing = layoutParams.horizontalSpacing;
        } else {
    ...
    private void drawDebugInfo(Canvas canvas, View childView) {
        if (!debugDraw) {
            return;
        }
        ...
        if (lp.horizontalSpacing > 0) {
            float x = childView.getRight();
            float y = childView.getTop() + childView.getHeight() / 2.0f;
        ...
        if (lp.verticalSpacing > 0) {
            float x = childView.getLeft() + childView.getWidth() / 2.0f;
            float y = childView.getBottom();
        ...
        if (lp.newLine) {
            if (orientation == HORIZONTAL) {
                float x = childView.getLeft();
                float y = childView.getTop() + childView.getHeight() / 2.0f;
                canvas.drawLine(x, y - 6.0f, x, y + 6.0f, newLinePaint);
            } else {
                float x = childView.getLeft() + childView.getWidth() / 2.0f;
                float y = childView.getTop();
